...
% numcore   : number of core for parallel processing. It changes computer
%             capacity. You can use 4 core.
%
% OUTPUT    :
% This function create a text file with extention ".art". The art file is
% result of 2D ray-tracing delays.
%
% Coded by Kamil Teke and Ozgur Ozel [26 Dec 2025]
% ************************************************************************

function raytracer_parallel(matfile,statfile,azimuth,elevation,numcore)
format longg
addpath('UNDULATIONS');
...
[y, m, d, ho, minu, sec_val] = mjd2date(tmjd);
datestr_mat = [y m d ho minu sec_val];
clear y m d ho minu sec_val

q_vals = q.val;
q_scale = q.scale_factor;
q_offset = q.add_offset;

z_vals = z.val;
z_scale = z.scale_factor;
z_offset = z.add_offset;

t_vals = t.val;
t_scale = t.scale_factor;
t_offset = t.add_offset;

level_vals = level.val;

N08_grid = N08.geoid_grid;

stat_data = stat;
if isempty(gcp('nocreate'))
    parpool(numcore);
end
parfor iep = 1 : length(tmjd)
    curr_mjd = tmjd(iep);
    curr_datestr = datestr_mat(iep,:);
    local_fileName2 = sprintf('OUTPUT/%s_%04d%02d%02d_%02d.art',matfile(1:end-4),curr_datestr(1),curr_datestr(2),curr_datestr(3),curr_datestr(4));
    art = fopen(local_fileName2,'w+');
    ...
    mjd = curr_mjd;
    disp(curr_datestr) % display processing MJD
    spechum = squeeze(double(q_vals(:,:,:,iep)).*q_scale + q_offset); % specific humidity [kg*kg^-1]
    geopot = squeeze(double(z_vals(:,:,:,iep)).*z_scale + z_offset); % geopotential [m^2/s^2]
    tempk = squeeze(double(t_vals(:,:,:,iep)).*t_scale + t_offset); % temperature [K]
       
    % converting the geopotential heights to orthometric heights
    geomh = nan(numlon,numlat,numlevel);
    geoid_grid = N08_grid;
    for ilon_idx = 1 : numlon
        for ilat_idx = 1 : numlat
            geomh(ilon_idx,ilat_idx,:)= geopot2geomh(squeeze(geopot(ilon_idx,ilat_idx,:)),geodlat(ilat_idx)); % [m]
            geomh(ilon_idx,ilat_idx,:) = geomh(ilon_idx,ilat_idx,:) + geoid_grid(ilat_idx,ilon_idx); % converting from orthometric to ellipsoidal heights of all profiles
        end
    end
    ...
    totpress = double(level_vals)*1e2; % total pressure [Pa]
    ...
    for ilon = 1 : numlon
        for ilat = 1 : numlat
            ...
            elseif opt == 2
                [presstopointerp,wvptopointerp] = interpgridpresswvp(hinterp(find(hinterp==minhinterp):end)',squeeze(geomh(ilon,ilat,:)),squeeze(press(ilon,ilat,:)),squeeze(wvp(ilon,ilat,:))); % pressure and wvp in hPa at 929 levels (interpolated)
        ...
        for iaz = 1 : length(azimuth) 
            for ielv = 1 : length(elevation) 
                ...
                fprintf(art,['%6d %11.5f %d %3d %2d %2d %5.2f %-8s %20.15f '...
                    ...
                    count,mjd,curr_datestr(1),yyDoySecond(2),curr_datestr(4),curr_datestr(5),curr_datestr(6),...
                    stat_data(istat).id,deg2rad(azimuth(iaz)),deg2rad(elevation(ielv)),src,t_meas,p_meas,wvp_meas,...
